...
| 各省部 Agent | **Skills**：`edict-opencode/skills/` 下 `taizi`、`zhongshu`、`menxia`、`shangshu`、`libu`、`hubu`、`bingbu`、`xingbu`、`gongbu`、`libu_hr`、`zaochao`（钦天监）等。 |
...
- **shangshu** → `["libu", "hubu", "bingbu", "xingbu", "gongbu", "libu_hr", "zaochao"]`
- **六部 + 钦天监**（libu, hubu, bingbu, xingbu, gongbu, libu_hr, zaochao）→ `[]`（不越权调用其他 agent）
