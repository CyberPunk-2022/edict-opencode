...
|---------------|----------------|
| DB / EventBus | **文件**：项目内 `edict-opencode/edict-tasks.json`（或安装后项目根下 `edict/edict-tasks.json`），结构照抄 edict 的 Task Schema（`state` / `org` / `flow_log` / `progress_log` / `_scheduler` 等）。 |
...
edict-opencode/
  ...
  edict-tasks.json             # 本地任务状态（可作模板，实际可放在项目根或 edict/ 下）
  README.md                    # 安装与使用说明
  install.sh                   # macOS/Linux 一键安装
  install.ps1                  # Windows 一键安装（可选）
```

说明：运行期 `edict-tasks.json` 可放在「当前工作项目」根目录下的 `edict/edict-tasks.json`，由插件或脚本通过配置/约定路径读取，以便每个项目一套任务看板。
...
3. **可选**：自动复制 `edict_tasks_api.py` 到当前项目 `scripts/`，便于在 skills 中固定调用 `python scripts/edict_tasks_api.py ...`。
...
2. **建议优先落地的三份文件**：
   ...
   - `edict-opencode/scripts/edict_tasks_api.py`（读写 edict-tasks.json、状态推进、flow/progress 追加、can_dispatch_to）。
